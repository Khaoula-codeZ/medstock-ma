    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">Suivi des pénuries au Maroc</p>
        </div>
        <Link
          to="/report"
          className="shrink-0 px-3 py-2 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700"
        >
          + Signaler
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <h2 className="text-sm font-semibold text-gray-700 mb-3">Pénuries par catégorie</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <h2 className="text-sm font-semibold text-gray-700 mb-3">Répartition par sévérité</h2>
          <ResponsiveContainer width="100%" height={180}>
            <PieChart>
              <Pie
                data={severityData}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/shortages" className="text-blue-600 text-xs hover:underline">Voir tout →</Link>
        </div>
        <div className="space-y-2">
          {recentShortages.length === 0 ? (
            <p className="text-center text-gray-400 text-sm py-6">Aucun signalement pour l'instant</p>
          ) : recentShortages.map(shortage => (
            <div key={shortage.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{shortage.drug_name}</p>
                  <p className="text-xs text-gray-500 truncate">{shortage.wilaya} · {shortage.category}</p>
                </div>
              </div>
